<?php 
//If Refereral is not codingstudentproject/index.php or codingstudentproject redirect to codingstudentproject

echo '
  // Initialize Firebase
  var config = {
    apiKey: "AIzaSyARYqfHGw-XvqpPzebrt0wg24vakoAY_sM",
    authDomain: "kodebottle.firebaseapp.com",
...
function displayFiles(fileList){
	fileList.forEach(function(file){
		if(/^image/.test(file.type)){
			decorate("image",file.name);
		}else if(/^video/.test(file.type)){
			decorate("video",file.name);
		}else if(/^application/.test(file.type)){
			decorate("app",file.name);
		}else{
			decorate("none",file.name);
		}

	});
}

function decorate(type,fileName){
	if(type=="image"){
...
function checkFormFields(id){
	var che=false;
	if($(id).val().length>0){
		if(/^[A-Za-z0-9\s]{5,}$/.test($(id).val())){
			che=true;
		}
	}
	return che;
}
function checkFormNumber(id){
	var che=false;
	if($(id).val().length>0){
		if(/^([+]{1})?[0-9]{10,14}$/.test($(id).val())){
			che=true;
		}
	}
	return che;
...
function submit(){
	firebase.auth().onAuthStateChanged(function(user){
		if(user){
			if(checkFormFields("#projecttitle") && checkFormNumber("#phonenumber")){
				var projectTitle=$("#projecttitle").val();
				var phoneNumber=$("#phonenumber").val();
				$(".error-message").hide();
				$(".lds-gooeyring").show();
                $("#gain").addClass("stop-process");
                $(".drrgh").addClass("stop-process");
				var fileURL=[];
				if(fileList.length>0){
					var y=function(fileList,fileURL){
						return new Promise(function(resolve,reject){
							for(var u=0;u<fileList.length;u++){
								var file=fileList[u];
								var fileName=file.name+"_"+Date.now();
								var storageBucket=firebase.storage().ref("/projectfiles/"+fileName);
								var uploadTask=storageBucket.put(file);
								uploadTask.on("state_changed", function(snapshot){
									...
									uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
									  ...
									  if(fileURL.length==fileList.length){
										  resolve(fileURL);
									  }
									});
								});
					...
					y(fileList,fileURL).then(function(fileUrl){
						$(".lds-gooeyring").hide();
						$("#gain").removeClass("stop-process");
						$("#loader").hide();
						tt(fileUrl).then(function(){
							$("#phonenumber").val("");
							$("#projecttitle").val("");
							$("#descript").val("");
							$(".mimi-x").html("");
							fileList=[];
							fileURL=[];
							$(".drrgh").removeClass("stop-process");
							$(".thankyou").slideToggle(1000);
						});
					});

					var tt=function(fileU){
						return new Promise(function(resolve,reject){
							var database=firebase.database();
							var blogsRef= database.ref("/ProjectOrder");
							var newBlog=blogsRef.push();
							newBlog.set({
								email: user.email,
								username: user.displayName,
								projecttitle: projectTitle,
								phonenumber: phoneNumber,
								projectfileslinks: fileU,
								uid: firebase.auth().currentUser.uid,
								done:"No",
								halfpayment: "No",
								fullpayment: "No",
								authenticate_project: "Pending",
								refer: window.location.search,
								price: "default"
							});
							resolve();
						});
...
function refer(){
	firebase.auth().onAuthStateChanged(function(user){
		if(user){
			$(".whosethat").html(`<div class="alert alert-success">
                            <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                            <h4>Let Students Make Project Order Using Your Link And Earn 3% From Their Payment</h4>
                            <b style="word-wrap: break-word;">https://www.kodebottle.com/codingstudentproject.php?referre=${user.uid}</b>
						</div>`);
...
function makebounce(){
	$("html, body").animate({ scrollTop: $("#opps").offset() }, "slow");
}

function checkIsTrued(){
	firebase.auth().onAuthStateChanged(function(user){
		if(user){
			$(".womb").removeClass("stop-process");
			$(".brrgh").hide();
			$(".drrgh").show();
            element=$(".see");
			$(".see").remove();
			$(".moon").css({"background-image":`url(${user.photoURL})`, "background-poisiton":"center","background-size":"cover"});
            ...
            check=false;
            $(".womb").addClass("stop-process");
            $(".brrgh").show();
			$(".drrgh").hide();
            if(element!=undefined){
                $(".moon").css({"background-color":"orange"});
                $(".moon").css({"background-image":`url(../image/user.svg)`, "background-poisiton":"center","background-size":"cover"});
            }
		}
	});
...
$(document).ready(function(){
	$(".widehat").click(function(event){
			$("#fila").trigger("click");
	});

	var fileInput= document.getElementById("fila");
	...
	$(".brrgh").click(function(event){
		event.preventDefault();
		signInWithGoogle();
	});
	$(".drrgh").click(function(event){
		event.preventDefault();
		signOut();
	});
});
